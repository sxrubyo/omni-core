...
class SyncRuntimeOpsTests(unittest.TestCase):
    ...
    def test_build_remote_sync_command_supports_file_sources(self):
        with tempfile.TemporaryDirectory() as tmp:
            ssh_dir = Path(tmp) / ".ssh"
            ssh_dir.mkdir()
            private_key = ssh_dir / "n8n_nova"
            private_key.write_text("PRIVATE", encoding="utf-8")
            target_dir = Path(tmp) / "restore-parent"
            ...
            self.assertIn("ubuntu@172.31.34.176:", command)
            self.assertIn("/home/ubuntu/.bash_history", command)
            self.assertNotIn("/home/ubuntu/.bash_history/", command)

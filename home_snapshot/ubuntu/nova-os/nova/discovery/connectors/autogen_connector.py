...
class AutoGenConnector(BaseAgentConnector):
    """Thin Python-wrapper connector for AutoGen runtimes."""

    connector_name = "autogen"

    async def connect(self, agent: DiscoveredAgent) -> ConnectionResult:
        available = self._import_module("autogen") or self._import_module("autogen_agentchat")
        return ConnectionResult(
            ...
            error=None if available else "AutoGen is not importable",
        )

    async def health_check(self, _: DiscoveredAgent) -> HealthStatus:
        available = self._import_module("autogen") or self._import_module("autogen_agentchat")
        return HealthStatus(ok=available, status="online" if available else "offline", detail="autogen import")

    async def get_status(self, agent: DiscoveredAgent) -> dict[str, object]:
        return {"connector": self.connector_name, "module_available": self._import_module("autogen"), "pid": agent.pid}
    ...
    async def send_task(self, agent: DiscoveredAgent, task: AgentTask) -> TaskResult:
        ...
        if not entrypoint:
            return TaskResult(success=False, error="AutoGen tasks require payload.entrypoint or agent metadata entrypoint")

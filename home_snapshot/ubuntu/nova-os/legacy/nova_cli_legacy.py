...
_AGENT_REGISTRY: dict = {
    ...
    "openclaw": {
        ...
        "process_kw":   ["openclaw", "openclaw-gateway", "openclaw-onboard"],
        # Method 3: home dirs
        "home_dirs":    [str(Path.home() / ".openclaw"), str(Path.home() / ".openclaw/workspace")],
        # Method 4: shell vars (exported)
        "shell_vars":   ["OPENCLAW_URL", "OPENCLAW_PORT", "OPENCLAW_API_KEY", "OPENCLAW_GATEWAY_PORT", "OPENCLAW_SERVICE_KIND"],
        # Method 5: packages
        "npm_pkg":      "openclaw",
        "pip_pkg":      None,
        # Port probe
        ...
        "env_port_hint": {"OPENAI_BASE_URL": 18789, "OPENCLAW_GATEWAY_PORT": 18789},
        # Config files in project
        "project_files": [".openclaw", "openclaw.config.json", "openclaw.yaml", "AGENTS.md", "SOUL.md"],
        # Integration
        "integration":  ["proxy", "alias", "governance"],
...
_CMD_NEEDS_CORE = {
    ...
    "protect", "scan", "connect", "boot", "search",
}
_CMD_NEEDS_API = {
...
# ══════════════════════════════════════════════════════════════════════════════
# SEARCH AGENTS — Enhanced discovery with deep scanning
# ══════════════════════════════════════════════════════════════════════════════

def cmd_search(args):
    """
    Deep search for AI agents — enhanced version of scan.
    Finds agents by: ports, processes, binaries, config files, env vars.
    """
    print_logo(compact=True)
    section("Deep Agent Search")
    ...
    # ── Method 1: Port Scan (via Nova Core) ───────────────────────────────────
    print("  " + q(C.G2, "[1/4] Scanning ports..."))
    nc = _get_nova_core()
    if nc.is_alive():
        result = nc.scan()
    ...
    print("  " + q(C.G2, "[2/4] Scanning processes..."))
    try:
        import subprocess
        ...
        known_processes = {
            "openclaw": ("OpenClaw", "python", 18789),
            "melissa": ("Melissa", "python", 8001),
            "n8n": ("n8n", "node", 5678),
            "ollama": ("Ollama", "native", 11434),
        }
        for proc_name, (display, runtime, default_port) in known_processes.items():
            if proc_name in ps_output.lower():
                already_found = any(
                    ag.get("name", "").lower() == display.lower() 
                    for ag in found_agents
                )
                if not already_found:
                    found_agents.append({
                        "method": "process",
                        "name": display,
                        "url": f"http://localhost:{default_port}" if default_port else "N/A",
                        "port": default_port,
                        "runtime": runtime,
                        "confidence": 60,
                    })
        ...
        if verbose_mode:
            warn(f"Process scan error: {e}")
    
    # ── Method 3: Binary/CLI Detection ────────────────────────────────────────
    print("  " + q(C.G2, "[3/4] Checking binaries..."))
    import shutil
    known_binaries = {
        "openclaw": ("OpenClaw", "server", 18789),
        "gemini": ("Gemini CLI", "cli", None),
        "claude": ("Claude Code", "cli", None),
        "n8n": ("n8n", "server", 5678),
    }
    for binary, (display, agent_type, default_port) in known_binaries.items():
        if shutil.which(binary):
            ...
            if not already_found:
                found_agents.append({
                    "method": "binary",
                    "name": display,
                    "url": f"http://localhost:{default_port}" if default_port else "CLI only",
                    "port": default_port,
                    "runtime": "mixed",
                    "confidence": 70,
                    "type": agent_type,
    ...
    # ── Method 4: Config Files & Home Dirs ────────────────────────────────────
    print("  " + q(C.G2, "[4/4] Checking config files..."))
    from pathlib import Path
    home = Path.home()
    config_patterns = {
        "OpenClaw": [".openclaw", ".openclaw/workspace/AGENTS.md"],
        "Melissa": ["melissa/", "melissa.env"],
        "Claude Code": [".claude/", ".claude/settings.json"],
        "Gemini CLI": [".gemini/", ".gemini/history/"],
        "n8n": [".n8n/", ".n8n/config"],
    }
    for agent_name, patterns in config_patterns.items():
        for pattern in patterns:
            path = home / pattern
            if path.exists():
                already_found = any(
                    ag.get("name", "").lower() == agent_name.lower() 
                    for ag in found_agents
                )
                if not already_found:
                    found_agents.append({
    ...
    if not found_agents:
        warn("No agents found. Try running your agent first.")
        print()
        hint("Then run:  nova search agents")
        return
    ...
    print("  " + q(C.W, f"Found {len(unique_agents)} agent(s):", bold=True))
    print()
    ...
    for ag in sorted(unique_agents, key=lambda x: -x["confidence"]):
        confidence_color = CONFIDENCE_COLORS.get(ag["confidence"], C.W)
        method_badge = {
            "port": "🔌",
            "process": "⚙️",
            "binary": "📦",
        ...
        print(
            "  " + q(C.GRN, "●") + "  " +
            q(C.W, ag["name"], bold=True) + "  " +
            q(confidence_color, f"({ag['confidence']}% confidence)") + "  " +
            q(C.G2, method_badge)
        )
        print("       " + q(C.B7, ag["url"]))
        if verbose_mode and ag.get("config_path"):
            print("       " + q(C.G3, f"Config: {ag['config_path']}"))
    
    print()
    
    # ── Quick Actions ─────────────────────────────────────────────────────────
    if len(unique_agents) > 0:
        print("  " + q(C.W, "Quick Actions:", bold=True))
        print()
        print("    " + q(C.G2, "nova connect") + "          Connect Nova Core to an agent")
        print("    " + q(C.G2, "nova agent create") + "     Create governance agent")
        print("    " + q(C.G2, "nova boot") + "             Start Nova + connect all")
        print()

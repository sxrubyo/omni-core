...
const openaiLogo = new URL('../nova-branding/ai-logos/openai-2025.svg', import.meta.url).href
const anthropicLogo = new URL('../nova-branding/ai-logos/anthropic.png', import.meta.url).href
const geminiLogo = new URL('../nova-branding/ai-logos/google-gemini.svg', import.meta.url).href
...
function Dashboard() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-8 pb-20 xl:pr-[480px]">
      ...
      <OperatorAssistant
        workspaceName={workspace?.name || 'Nova workspace'}
        ...
        <div className="grid gap-6 px-7 py-7 md:px-9 md:py-9 xl:grid-cols-[1.02fr_0.98fr]">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-white/68">
              Nova is strongest when the operator can understand what changed, what needs attention, and which model is being used. This dashboard is now structured around those decisions.
            </p>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              <HeroSignal
                label="Workspace"
                value={workspace?.name || 'Nova workspace'}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <motion.section variants={item} className="rounded-[30px] border border-black/8 bg-[linear-gradient(135deg,#fffdfa_0%,#f4eee3_100%)] p-6 shadow-[0_25px_70px_-55px_rgba(0,0,0,0.35)] dark:border-transparent dark:bg-[linear-gradient(135deg,#131921_0%,#10151b_100%)] dark:shadow-[0_32px_80px_-52px_rgba(0,0,0,0.82)]">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <ProviderSurface
              title="Server-ready providers"
              description="These are the primary providers the workspace can expose cleanly in the assistant."
              logos={[
            ...
            <ProviderSurface
              ...
              description="When there is no server key configured, the operator can still talk to these providers with a personal API key."
              chips={['Groq', 'xAI', 'Mistral', 'DeepSeek', 'Cohere']}
              tone="light"
            />
          </div>
...
function ProviderSurface({ title, description, logos = [], chips = [], tone = 'light' }) {
    ...
    <div className={`rounded-[26px] border p-5 ${classes}`}>
      ...
      {logos.length > 0 && (
        <div className="mt-5 flex flex-wrap gap-3">
          {logos.map((provider) => (
            <div key={provider.name} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 dark:border-white/10 dark:bg-white/[0.05]">
              <img src={provider.logo} alt={`${provider.name} logo`} className="h-6 w-auto object-contain" />

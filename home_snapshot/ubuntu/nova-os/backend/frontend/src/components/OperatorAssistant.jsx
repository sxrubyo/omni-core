...
const starterPrompts = [
  ...
  'Which Nova CLI commands should I run first?',
]
...
function OperatorAssistant({ onCreateAgent, onRefresh, workspaceName = 'Nova workspace' }) {
  ...
  const [messages, setMessages] = useState([
    {
      id: 'welcome',
      role: 'assistant',
      content: `Nova Operator is live for ${workspaceName}. Ask for alerts, risk, agent setup, CLI commands, or how to operate this workspace.`,
      suggestedCommands: ['nova status', 'nova watch'],
      actions: [
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content: response.success
            ...
            : `Command execution failed (code ${response.exit_code}).\n\n${response.output}`,
          meta: 'terminal-output',
        },
      ])
    } catch (err) {
      setMessages((prev) => [
              ...
              <div className={`border-b px-5 py-4 ${ui.line}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <h2 className="mt-1 text-lg font-semibold tracking-[-0.03em]">Nova Operator</h2>
                    <p className={`mt-2 text-sm ${ui.subtext}`}>
                      Ask Nova about alerts, risk, agent setup, commands, and workspace operations.
                    </p>
                  </div>
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center gap-2.5">
                                    <div className={`flex h-9 w-9 items-center justify-center rounded-2xl ${ui.providerLogo}`}>
                                      <img src={provider.logo} alt={provider.label} className="h-5 w-5 object-contain" />
                                    </div>
                                    <div className="min-w-0">
                ...
                >
                  ...
                  <div className={`rounded-[24px] p-3 ${ui.textareaWrap}`}>
                    <textarea
                      ...
                      rows={3}
                      placeholder="Ask Nova about alerts, policies, runtime status, or the next command to run."

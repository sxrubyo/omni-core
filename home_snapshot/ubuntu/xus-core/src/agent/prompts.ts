import type { TimeContext, LongTermMemory, InstructionPatch } from "../types/index.js";

export function buildSystemPrompt(timeCtx: TimeContext, ltm: LongTermMemory, patches: InstructionPatch[] = []): string {
...
function buildLTMSummary(ltm: LongTermMemory): string {
  if (!ltm || (!ltm.victories?.length && !ltm.clients?.length && !ltm.patterns?.length))
    return "Sin memoria de largo plazo registrada aún.";
  const lines: string[] = [];
  if (ltm.victories?.length) lines.push(`**Victorias recientes**: ${ltm.victories.slice(-3).map(v => v.content).join(" | ")}`);
  if (ltm.clients?.length) {
    const active = ltm.clients.filter(c => c.status === "active").slice(-5);
    if (active.length) lines.push(`**Clientes activos**: ${active.map(c => c.name).join(", ")}`);
  }
  if (ltm.patterns?.length) lines.push(`**Último patrón**: ${ltm.patterns[ltm.patterns.length - 1].content}`);
  return lines.join("\n");
}
